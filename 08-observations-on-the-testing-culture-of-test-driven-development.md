...
Uncle Bob has been the focus of the discipline for years, and it is natural to discuss him as a part of this write-up. Uncle Bob has defended TDD and has pushed the discipline’s boundaries.
...
The developer’s IDE (integrated developer environment) becomes a rubber duck begging for a conversation. At a minimum, TDD shops should be humming with this type of conversion.

> Think, then speak up about your immediate next move(s).

This reinforcement is key to communication, not only to predict your next action but also to reinforce the concepts of writing **straightforward** code to make a unit test pass. Of course, if the developer becomes silent, they are certainly wandering off the loop and must come back on the path.

## 2. TDD Commands Muscle Memory

As a developer cycles through TDD, they will experience fatigue by battling through high friction and awkward flow. The fatigue is expected as we pratice. The developer will find shortcuts to improve the cycle because the goal is to reduce that awkwardness and to enhance muscle memory.
...
Each time a developer practices TDD, they draw a mental map on what is to be solved. In a traditional coding approach, this is not always true, as the solution is exploratory. The developer may have a goal. To get to that goal, unit tests are neglected in the process.

The start and end of the session is ritualized. First, think, and list. Test your options. List more. Then start, do, and then think. Check off. Repeat. Finally, think, and stop.

> Maintain your test list well. Check off items as you go. Never drive without one. Think!

The list may take time to formulate and is not in the cycle. However, it should be prepared before the revolutions start. If you don’t have one, you do not know where you're going. Always have a map.
...
The practice will be smooth with a **test list**, as described by Kent Beck. The test list directs solving each cycle. Once the test list is solved, the cycle stops on red with a failing test.

## 4. Communicate with Others

As the above test list is filled out, specific steps become blocked because the commitment of work was not clear. If they are generating a presumptive test list which has guesses about the missing requirement(s), the suggestion is to stop right there.
...
In TDD, understanding what to build based on the owner’s requirement(s) is the goal. If context is unclear, the test list will start to break down. That breakdown will require a conversation. And direct conversions can quickly turn into trust, respect, and a feedback loop. 
...
Kent identified this caveat in the book, [*TDD By Example*](https://www.safaribooksonline.com/library/view/test-driven-development/0321146530/). Concurrency and security are the two significant areas where TDD lacks. The developer must design concurrency into the system design separately.
...
`length > 1` is called a **degenerative implementation**. It is enough implementation to solve the problem at hand, but tells us nothing about the problem we are trying to solve.
...
No matter how frail unit tests are, they are a core necessity. They are required to turn **fear** into **courage**. Tests allow us to refactor code mercifully. Tests guide and **document**, adding value to a project so others can contribute effectively.
...
In TDD, there are situations where we are entangled by the implementation. At some point, the testing code becomes a bottleneck to move forward — an **impasse** forms. The developer has to back out and disarm by removing a portion of the tests to get out.

Uncle Bob has experienced these impasses in his career, realizing making a test pass prefers an order to avoid an impasse. He called it a premise. **As the tests get more specific, the code gets more generic.**

![The order of the transformations. One should always lower-ordered changes.](images/08-03.png)

Uncle Bob created the [Transformation Priority Premise](https://8thlight.com/blog/uncle-bob/2013/05/27/TheTransformationPriorityPremise.html). The list defines an order of refactoring to achieve by passing a test. One must prefer a transformation that will incur the least risk to create an impasse.
...
Before we conclude, I’d like to answer my question, “what percentage of software professionals use TDD today?” My answer: is "I think the group is small." I'll explore this answer with reasons why.
...
The definition of a testing culture is a place where developers are deliberately practicing testing. They are mentoring those who are not skilled in the area. Each pairing and in every pull request is a feedback loop on building an individual's confidence, supported by an engineering leadership. In this culture, managers believe in testing. When deadlines and times get tough, the test discipline is not dropped — it is strengthened.

Those that have lived a testing culture, are lucky. We apply the experience to future projects, educating others.
...
The threshold for clean test execution is high, in that it eclipses a baseline of experience. The majority will hardly reach experience — from a testing culture.
...
> TDD discipline has struggled to take off due in part to the learning curve of testing. TDD requires a headspace that is unique and challenging.

TDD demands **thought** and **experience**, and more. It is not easy and is a skill. The practice commands the developer’s throughput, continuously and relentlessly. We are all **vulnerable** in the process, and few developers like being in this position.
