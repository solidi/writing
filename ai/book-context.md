...
- 2026-05-17: Spawned [book-editing.md](book-editing.md) as a production-side companion spoke. This file covers drafting and shaping a book-form artifact; the new file covers auditing a typeset or published PDF for index page-number errors, alphabetical-order slips in the index, endnote numbering drift, and bibliography order. Index discipline rules in this file are the contract that the editing spoke audits against. UTS-specific particulars captured in book-editing's body: pypdf workflow, printed-page → PDF-page mapping (UTS printed p.1 = PDF idx 30, endnotes printed 425–452, index printed 453–465), ligature mangling pitfalls (`ﬀ` / `ﬃ`), sub-entry parser leakage, and the family-before-individual / parent-before-specific / series-order conventions that look like alpha violations but are not.
