...
- 2026-06-06: Delta from essay 107 author edits ("The 5x Engineering Manager"). Coinage-essay patterns worth keeping:
  - **Small-caps bold opener belongs at every H2 section, not just essay top.** Multi-section pieces gain pacing when each section opens with `**WORD WORD**, body…`. Essay 107 used `**EARLY IN MY CAREER**`, then `NOW I SIT`, then `IN THE END` at the head of each section. Treat the opener as a section-level device, not a one-shot.
  - **Short essays can carry 2 H2 sections.** Even at ~600 words, when the thesis pivots (frame → frame-break), a header earns its keep. "No subtitle" in a prompt means no tagline under the title — it does not forbid H2s. Default to 1–2 H2s on any reflective piece over ~400 words.
  - **Compare-contrast paragraphs prefer comparative joiners over staccato A/B.** In an explicit compare-contrast essay, "A 10x engineer ships. A 5x manager hires." reads as drum-machine; "While a 10x engineer ships, a 5x manager hires the next ten." reads as thought. Reserve the staccato break for emphasis at the end of a section or the close of the piece. The compare-contrast structure is already doing the rhythmic work; do not double it with sentence chop.
  - **Collective opener beats narrator-singular for trope essays.** When the subject is an industry trope ("the 10x engineer", "the rockstar PM", "the architect"), open with "my team had one" / "we all knew the behaviors" instead of "I sat next to one". The trope is shared cultural property; the first-person plural earns permission to comment on it. Save the first-person singular for the pivot into the author's own role.
  - **Modernize tropes for the agentic era.** "Already read the code" → "already has agents writing the code." Old lines about Stack Overflow / compilation / IDE shortcuts read as 2014. When a 10x / greybeard / efficiency line lands flat, ask whether the agent-assisted version of the same observation is sharper.
  - **Skip the worn 10x clichés.** "Answers Stack Overflow while compiling", "refactors the build on Friday afternoon out of boredom" — the first is dead, the second still lives. Replace tired clichés with a behavior the author actually observed ("led a last-minute leadership initiative"). The specific beats the generic.
  - **Recursive coinage at the close beats frame-collapse.** Coinage essays (Springboard, SECTA, Claude's Law, 5x Manager) tempt the writer to crash the frame at the end ("Except that is not what the 5x manager actually is"). The published 107 instead tees up a *higher* tier — "perhaps there are 10x managers out there, after all" — and admits the author lives inside the joke. Recursive escalation lands warmer than rug-pull negation.
  - **Don't pad with HN link-bait when "online threads debate" carries the same weight.** A specific HN thread URL inside a reflective essay reads as receipt-flashing. Coinage essays earn their HN links when the essay *is* a reply to a thread (102). Reflective essays do not.
